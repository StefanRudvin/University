...
public class Tableau {
	...
	private boolean invRule(OWLIndividual node) {
		...
		HashMap<OWLObjectPropertyExpression, HashSet<OWLIndividual>> elmedges = edges.get(node);

		for (HashMap.Entry<OWLObjectPropertyExpression, HashSet<OWLIndividual>> entry : elmedges.entrySet()) {
			OWLObjectPropertyExpression property = entry.getKey();
			HashSet<OWLIndividual> values = entry.getValue();

			if(property.getInverses(ontologies).size() > 0){
				OWLIndividual subject = node;
				HashSet<OWLIndividual> object = values;
				OWLObjectPropertyExpression r = property;
				Set<OWLObjectPropertyExpression> s = property.getInverses(ontologies);

				System.out.println("We have a link from subject " + subject + " of " + r + " to " + object);
				System.out.println("So we should add a link from " + object + " of " + s + " to " + subject);

				ArrayList<OWLIndividual> objectlist = new ArrayList<OWLIndividual>(object);


				HashMap<OWLObjectPropertyExpression, HashSet<OWLIndividual>> objectedge = edges.get(objectlist.get(0));
